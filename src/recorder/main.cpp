...
int main() {
  ...
  UserWindowActivityEventSource &windowActivityEventSource =
      UserWindowActivityEventSource::getInstance();
  ...
  while ((ret = GetMessage(&msg, NULL, WM_KEYFIRST, WM_MOUSELAST)) != 0) {
    if (ret == -1) {
      throw std::runtime_error(
          "Error retrieving message with GetMessage(), hWnd is probably an "
