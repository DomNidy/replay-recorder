...
void UserInputEventSource::initializeSource(EventSink *inSink) {
  ...
  if (!hKeyboardHook) {
    throw std::runtime_error("Failed to register keyboard hook: " +
                             std::to_string(GetLastError()));
  }

  std::cout << "UserInputEventSource successfully installed keyboard hook"
            << std::endl;
}
...
void UserInputEventSource::uninitializeSource() {
  ...
  hKeyboardHook = NULL;
  std::cout << "UserInputEventSource successfully uninstalled keyboard hook"
            << std::endl;
}
...
LRESULT CALLBACK UserInputEventSource::KeyboardProc(int nCode, WPARAM wParam,
                                                    LPARAM lParam) {
  if (nCode == HC_ACTION) {
    ...
    if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) {
      ...
      // Handle special key or everything else
        ...
        if ((pKeyboard->vkCode >= 0x30 &&
             pKeyboard->vkCode <= 0x39) ||  // Numbers
            (pKeyboard->vkCode >= 0x41 &&
             pKeyboard->vkCode <= 0x5A) ||  // Letters
