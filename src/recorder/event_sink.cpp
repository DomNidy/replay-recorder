...
EventSink::EventSink(const std::string &name) {
  ...
  if (!file.is_open()) {
    throw std::runtime_error("Failed to open output file for EventSink - " +
...
EventSink &EventSink::operator<<(const char *data) {
  ...
  if (len == 0) {
    std::cerr
        << "Failed to calculate buffer size for UTF-8 to UTF-16 conversion: "
        << GetLastError() << "\n";
    return *this;
  }
  ...
  recordingBuffer.insert(recordingBuffer.end(), output.get(),
                         output.get() + len - 1);
...
inline void EventSink::_flushData() {
  if (!recordingBuffer.empty()) {
    ...
    try {
      std::string text =
          converter.to_bytes(recordingBuffer.data(),
                             (recordingBuffer.data() + recordingBuffer.size()));
